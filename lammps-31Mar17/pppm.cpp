...
PPPM::PPPM(LAMMPS *lmp, int narg, char **arg) : KSpace(lmp, narg, arg), 
  ...
  part2grid(NULL), boxlo(NULL),density_image_brick(NULL),density_real_brick(NULL),density_image_fft(NULL),
  density_real_fft(NULL),work_image(NULL),work_real(NULL),rho1d_image(NULL),remap_image(NULL),part2grid_image(NULL),
  A_rl(NULL),A_im(NULL),B_rl(NULL),B_im(NULL),pre_up(NULL),pre_down(NULL)
{
  ...
  density_brick =density_image_brick = density_real_brick = vdx_brick = vdy_brick = vdz_brick = NULL;
  density_fft = density_image_fft= density_real_fft = NULL;
  u_brick = NULL;
  v0_brick = v1_brick = v2_brick = v3_brick = v4_brick = v5_brick = NULL;
  greensfn = NULL;
  work1 = work2 =work_image= work_real=NULL;
  vg = NULL;
  fkx = fky = fkz = NULL;
  ...
  gf_b = NULL;
  rho1d = rho_coeff = drho1d = drho_coeff = rho1d_image=NULL;

  fft1 = fft2 = NULL;
  remap = remap_image=NULL;
  cg = NULL;
  cg_peratom = NULL;

  nmax = 0;
  part2grid = NULL;
  part2grid_image=NULL;
...
PPPM::~PPPM()
{
  ...
  if (imageflag==1) memory->destroy(part2grid_image);
    ...
    for (j = nylo_fft; j <= nyhi_fft; j++) {
      for (i = nxlo_fft; i <= nxhi_fft; i++) {
			...
			for(temp_count=0;temp_count<order_image_long;temp_count=temp_count+2){
			    tempA_rl=tempA_rl+(pre_down[temp_count]+pre_up[temp_count])*cos(fkz[k]*(1+temp_count)*dielectric_interface_L);
				  tempA_im=tempA_im+(-pre_down[temp_count]+pre_up[temp_count])*sin(fkz[k]*(1+temp_count)*dielectric_interface_L);
			}
			for(temp_count=1;temp_count<order_image_long;temp_count=temp_count+2){
			    tempB_rl=tempB_rl+(pre_down[temp_count]+pre_up[temp_count])*cos(fkz[k]*(1+temp_count)*dielectric_interface_L);
				  tempB_im=tempB_im+(-pre_down[temp_count]+pre_up[temp_count])*sin(fkz[k]*(1+temp_count)*dielectric_interface_L);
			}
			A_rl[n]=tempA_rl;
			A_im[n]=tempA_im;
			B_rl[n]=tempB_rl;
...
void PPPM::compute(int eflag, int vflag)
{
  ...
  if (atom->nmax > nmax) {
	...
	if (imageflag==1) {
      memory->destroy(part2grid_image);
      memory->create(part2grid_image,nmax,3,"pppm:part2grid_image");
    }
  }
  ...
  density_brick = density_real_brick;
  density_fft = density_real_fft;
  cg->reverse_comm(this,REVERSE_RHO);
  brick2fft();
  
  //for image charge
  density_brick = density_image_brick;
  density_fft = density_image_fft;
  cg->reverse_comm(this,REVERSE_RHO);
  brick2fft();
...
void PPPM::allocate()
{
  ...
  memory->create(work_real,2*nfft_both,"pppm:work_real");
  memory->create(vg,nfft_both,6,"pppm:vg");
  memory->create3d_offset(density_real_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                            nxlo_out,nxhi_out,"pppm:density_real_brick");
  memory->create(density_real_fft,nfft_both,"pppm:density_real_fft");
  if(imageflag==1){
	  memory->create(A_rl,nfft_both,"pppm:A_rl");
	  memory->create(A_im,nfft_both,"pppm:A_im");
	  memory->create(B_rl,nfft_both,"pppm:B_rl");
	  memory->create(B_im,nfft_both,"pppm:B_im");
	  memory->create(pre_up,order_image_long*2,"pppm:pre_up");
	  memory->create(pre_down,order_image_long*2,"pppm:pre_down");
	  memory->create3d_offset(density_image_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                                nxlo_out,nxhi_out,"pppm:density_image_brick");
    memory->create(density_image_fft,nfft_both,"pppm:density_image_fft");
    memory->create(work_image,2*nfft_both,"pppm:work_image");
  }
  if (triclinic == 0) {
  ...
  if (imageflag==1) {
    memory->create2d_offset(rho1d_image,3,-order/2,order/2,"pppm:rho1d_image");
  }
  ...
  if (imageflag==1) {
    remap_image = new Remap(lmp,world,
                            nxlo_in,nxhi_in,nylo_in,nyhi_in,nzlo_in,nzhi_in,
                            nxlo_fft,nxhi_fft,nylo_fft,nyhi_fft,nzlo_fft,nzhi_fft,
                            1,0,0,FFT_PRECISION,collective_flag);
  }
...
{
  imageflag=force->kspace->imageflag;
  memory->destroy3d_offset(density_real_brick,nzlo_out,nylo_out,nxlo_out);
  if (imageflag==1) memory->destroy3d_offset(density_image_brick,nzlo_out,nylo_out,nxlo_out);
  if (differentiation_flag == 1) {
  ...
  memory->destroy(density_real_fft);
  memory->destroy(greensfn);
  memory->destroy(work1);
  memory->destroy(work_real);
  ...
  memory->destroy(A_im);
  memory->destroy(B_im);
  memory->destroy(pre_up);
  memory->destroy(pre_down);  
  }
  if (imageflag==1) {
    memory->destroy(density_image_fft);
    memory->destroy(work_image);
  }
  ...
  if (imageflag==1) memory->destroy2d_offset(rho1d_image,-order_allocated/2);
...
void PPPM::particle_map()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    part2grid[i][2] = nz;
	if (imageflag==1) {
      nz_image = static_cast<int> ((-x[i][2]-boxlo[2])*delzinv+shift) - OFFSET;
      part2grid_image[i][0] = nx;
      part2grid_image[i][1] = ny;
      part2grid_image[i][2] = nz_image;
    }
...
void PPPM::make_rho()
{
  int l,m,n,nx,ny,nz,mx,my,mz,nz_image,mz_image;
  FFT_SCALAR dx,dy,dz,x0,x0_image,y0,y0_image,z0,dz_image;
  imageflag=force->kspace->imageflag;
  dielectric_interface_L=force->kspace->dielectric_interface_L;
  order_image_short=force->kspace->order_image_short;
  order_image_long=force->kspace->order_image_long;
  ...
  memset(&(density_real_brick[nzlo_out][nylo_out][nxlo_out]),0,
         ngrid*sizeof(FFT_SCALAR));
  if (imageflag==1) memset(&(density_image_brick[nzlo_out][nylo_out][nxlo_out]),0,
           ngrid*sizeof(FFT_SCALAR));//added
  ...
  for (int i = 0; i < nlocal; i++) {
	...
	if (imageflag==1) {
      nz_image=part2grid_image[i][2];
      dz_image=nz_image+shiftone - (-x[i][2]-boxlo[2])*delzinv;
      compute_rho1d_image(dx,dy,dz_image);
    }
    z0 = delvolinv * q[i];
    for (n = nlower; n <= nupper; n++) {
      mz = n+nz;
	  if (imageflag==1) mz_image = n+nz_image;
      y0 = z0*rho1d[2][n];
	  if (imageflag==1) y0_image=z0*rho1d_image[2][n];
      for (m = nlower; m <= nupper; m++) {
        my = m+ny;
        x0 = y0*rho1d[1][m];
		    x0_image=y0_image*rho1d_image[1][m];
        for (l = nlower; l <= nupper; l++) {
          mx = l+nx;
          density_real_brick[mz][my][mx] += x0*rho1d[0][l];
		    if (imageflag==1) density_image_brick[mz_image][my][mx] += x0_image*rho1d_image[0][l];
        }
      }
    }
  }
...
void PPPM::poisson_ik()
{
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    work_real[n] = density_real_fft[i];
    if (imageflag==1) {
      work_image[n]=density_image_fft[i];
    }
    n=n+1;
    work_real[n] = ZEROF;
    if (imageflag==1) {
  ...
  fft1->compute(work_real,work_real,1);
  if (imageflag==1) fft1->compute(work_image,work_image,1);
...
if (eflag_global || vflag_global) {
    if (vflag_global) {
      ...
      if (imageflag==0) {
        for (i = 0; i < nfft; i++) {
          eng = s2 * greensfn[i] * (work_real[n]*work_real[n] + work_real[n+1]*work_real[n+1]);
        ...
        for (i = 0; i < nfft; i++) {
          eng = s2 * greensfn[i] * ((work_real[n]*work_real[n]+ work_real[n+1]*work_real[n+1])*(1+B_rl[i])+
            (work_real[n]*work_image[n]+work_real[n+1]*work_image[n+1])*A_rl[i]+
            (work_real[n+1]*work_image[n]-work_real[n]*work_image[n+1])*A_im[i]);
          for (j = 0; j < 6; j++) virial[j] += eng*vg[i][j];
          if (eflag_global) energy += eng;
          n += 2;
        }
      ...
      if (imageflag==1) {
        for (i = 0; i < nfft; i++) {
          energy += s2 * greensfn[i] * ((work_real[n]*work_real[n]+ work_real[n+1]*work_real[n+1])*(1+B_rl[i])+
            (work_real[n]*work_image[n]+work_real[n+1]*work_image[n+1])*A_rl[i]+
            (work_real[n+1]*work_image[n]-work_real[n]*work_image[n+1])*A_im[i]);
          n += 2;
        }
        ...
        for (i = 0; i < nfft; i++) {
          energy +=s2 * greensfn[i] * (work_real[n]*work_real[n] + work_real[n+1]*work_real[n+1]);
          n += 2;
        }
      }
    }
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    work_real[n++] *= scaleinv * greensfn[i];
    work_real[n++] *= scaleinv * greensfn[i];
  }
  ...
  if (imageflag==1) {
    for (i = 0; i < nfft; i++) {
      work_image[n++] *= scaleinv * greensfn[i];
      work_image[n++] *= scaleinv * greensfn[i];
    }
  }
  ...
  if (imageflag==0) {
    ...
    for (k = nzlo_fft; k <= nzhi_fft; k++)
      for (j = nylo_fft; j <= nyhi_fft; j++)
        for (i = nxlo_fft; i <= nxhi_fft; i++) {
          work2[n] = fkx[i]*(work_real[n+1]*(1+B_rl[per])+work_image[n+1]*A_rl[per])
          +fkx[i]*(work_real[n]*0+work_image[n]*A_im[per]);
          work2[n+1] = -fkx[i]*(work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])+
          fkx[i]*(work_real[n+1]*0+work_image[n+1]*A_im[per]);
          n += 2;
		      per=per+1;
        }
    ...
    for (k = nzlo_fft; k <= nzhi_fft; k++)
      for (j = nylo_fft; j <= nyhi_fft; j++)
        for (i = nxlo_fft; i <= nxhi_fft; i++) {
          work2[n] = fky[j]*(work_real[n+1]*(1+B_rl[per])+work_image[n+1]*A_rl[per])
		      +fky[j]*(work_real[n]*0+work_image[n]*A_im[per]);
          work2[n+1] = -fky[j]*(work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])+
		      fky[j]*(work_real[n+1]*0+work_image[n+1]*A_im[per]);
          n += 2;
		      per=per+1;
        }
    ...
    for (k = nzlo_fft; k <= nzhi_fft; k++)
      for (j = nylo_fft; j <= nyhi_fft; j++)
        for (i = nxlo_fft; i <= nxhi_fft; i++) {
          work2[n] = fkz[k]*(work_real[n+1]*(1+B_rl[per])+work_image[n+1]*A_rl[per])
		      +fkz[k]*(work_real[n]*0+work_image[n]*A_im[per]);
          work2[n+1] = -fkz[k]*(work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])+
		      fkz[k]*(work_real[n+1]*0+work_image[n+1]*A_im[per]);
          n += 2;
		      per=per+1;
        }
...
void PPPM::poisson_peratom()
{
  ...
  if (eflag_atom && differentiation_flag != 1) {
    ...
    per=0;
    for (i = 0; i < nfft; i++) {
      work2[n] = (work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]);
      work2[n+1] = (work_real[n+1]*(1+B_rl[per])+work_image[n+1]*A_rl[per])
      +(work_real[n]*B_im[per]+work_image[n]*A_im[per]);
        n += 2;
        per=per+1;
      }
    fft2->compute(work2,work2,-1);
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    work2[n] = vg[i][0]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    work2[n+1] = vg[i][0]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    n += 2;
  }
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    work2[n] = vg[i][1]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    work2[n+1] = vg[i][1]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    n += 2;
  }
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    work2[n] = vg[i][2]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    work2[n+1] = vg[i][2]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    n += 2;
  }
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    work2[n] = vg[i][3]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    work2[n+1] = vg[i][3]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    n += 2;
  }
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    work2[n] = vg[i][4]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    work2[n+1] = vg[i][4]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    n += 2;
  }
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    work2[n] = vg[i][5]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    work2[n+1] = vg[i][5]*((work_real[n]*(1+B_rl[per])+work_image[n]*A_rl[per])-
      (work_real[n+1]*B_im[per]+work_image[n+1]*A_im[per]));
    n += 2;
  }
...
{
  int i,l,m,n,nx,ny,nz,mx,my,mz,nz_image,mz_image;
  FFT_SCALAR dx,dy,dz,dz_image,x0,y0,z0;
...
void PPPM::compute_rho1d_image(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                         const FFT_SCALAR &dz_image)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
