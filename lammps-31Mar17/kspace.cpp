...
KSpace::KSpace(LAMMPS *lmp, int narg, char **arg) : Pointers(lmp)
{
  ...
  slabflag = 0;
  imageflag=0;//flag for dielectric surface
  surface_chargeflag=0;
  differentiation_flag = 0;
  slab_volfactor = 1;
  dielectric_jump1 = 0;//for dielectric surface(up)
  dielectric_jump2 = 0;//for dielectric surface(down)
  dielectric_interface_L = 0;//the height of dielectric surface
  order_image_short = 0;//highest order for the truncation of short range part
  order_image_long = 0;//highest order for the truncation of long range part
  charge_up = 0;
  charge_down = 0;
...
void KSpace::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
	...
	kewaldflag = 0;
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"mismatch") == 0) {
      if (iarg+3 > narg) error->all(FLERR,"Illegal kspace_modify command");
      imageflag = 1;
      dielectric_jump1 = force->numeric(FLERR,arg[iarg+1]);
	    dielectric_jump2 = force->numeric(FLERR,arg[iarg+2]);
      iarg += 3;
    } else if (strcmp(arg[iarg],"interface_height") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal kspace_modify command");
      dielectric_interface_L = force->numeric(FLERR,arg[iarg+1]);
      iarg += 2;
    } else if (strcmp(arg[iarg],"order_image_charge") == 0) {
      if (iarg+3 > narg) error->all(FLERR,"Illegal kspace_modify command");
      order_image_short = force->inumeric(FLERR,arg[iarg+1]);
	    order_image_long  = force->inumeric(FLERR,arg[iarg+2]);
      iarg += 3;
    } else if(strcmp(arg[iarg],"interface_charge") == 0){
      if (iarg+3 > narg) error->all(FLERR,"Illegal kspace_modify command");
      surface_chargeflag=1;
      charge_up = force->numeric(FLERR,arg[iarg+1]);
      charge_down = force->numeric(FLERR,arg[iarg+2]);
      iarg += 3;
    } else if (strcmp(arg[iarg],"surface_energy_yes") == 0) {
      if (iarg+1 > narg) error->all(FLERR,"Illegal kspace_modify command");
      surface_energy_flag=1;
      iarg += 1;
    }
